...
import {
  ...
  Settings, HelpCircle, LogOut, Menu, X, ChevronLeft, ChevronRight
} from 'lucide-react';
...
function Sidebar({ page, setPage, collapsed, setCollapsed }: {
  ...
  return (
    <aside className={`hidden md:flex flex-col bg-[#1A2B1F] text-white transition-all duration-300 ${collapsed ? 'w-[70px]' : 'w-[240px]'} min-h-screen shrink-0 relative`}>
      {/* Logo area */}
      <div className="flex items-center justify-between px-4 py-5 border-b border-white/10">
        <OJLogo collapsed={collapsed} />
        {!collapsed && (
          <button onClick={() => setCollapsed(true)} className="text-white/40 hover:text-white cursor-pointer transition-colors ml-auto">
      ...
      <div className="flex flex-col gap-1 px-2 pb-4 border-t border-white/10 pt-3">
        ...
        {[
          ...
          <button key={item.label} title={collapsed ? item.label : undefined}
        ...
        {collapsed && (
          <div className="w-8 h-8 rounded-full bg-[#0E9F6E] flex items-center justify-center text-[12px] font-bold mx-auto mt-2">أ</div>
        )}
      </div>
    </aside>
  );
...
const PAGE_TITLES: Record<Page, string> = {
  dashboard: 'الداشبورد', employees: 'الموظفين',
  attendance: 'الحضور والغياب', payroll: 'الرواتب', accounting: 'الإيرادات والمصروفات',
};
const PAGE_SUBTITLES: Record<Page, string> = {
  dashboard: 'نظرة عامة على الشركة', employees: 'إدارة فريق العمل',
  attendance: 'متابعة الحضور اليومي', payroll: 'صرف الرواتب والمكافآت', accounting: 'متابعة الإيرادات والمصروفات',
};
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 bg-white border-b border-[#E8EFEA] px-5 sm:px-8 h-14 flex items-center justify-between shadow-[0_1px_8px_-2px_rgba(0,0,0,0.05)]">
          <div className="flex items-center gap-3">
            ...
            {collapsed && (
              <button onClick={() => setCollapsed(false)} className="hidden md:flex items-center justify-center w-8 h-8 rounded-lg hover:bg-[#F3F4F6] text-[#6B7280] hover:text-[#1A1A1A] transition-colors cursor-pointer">
          ...
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-[#0E9F6E] flex items-center justify-center text-white text-[13px] font-bold cursor-pointer">أ</div>
          </div>
        </header>
